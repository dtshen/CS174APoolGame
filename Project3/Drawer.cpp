...
void Drawer::initDrawer(GLuint program)
{
    ...
    generateCube(program, &m_cubeData, 1.0f);
...
//	generateSphere( program, &sphereData );
	...
	generateCube(program, &m_texCubeData, 0.1f);
	TgaImage floorImage;
	if(!floorImage.loadTGA("textures/tennis.tga"))
	{
		printf("Error loading image floor.tga\n");
	}
    glGenTextures( 1, &texture_floor );
    glBindTexture( GL_TEXTURE_2D, texture_floor );
    glTexImage2D(GL_TEXTURE_2D, 0, 4, floorImage.width, floorImage.height, 0,
                 (floorImage.byteCount == 3) ? GL_BGR : GL_BGRA,
                 GL_UNSIGNED_BYTE, floorImage.data );
...
void Drawer::setDrawerParam(mat4 m_view, shaderVariables vars, GLint uEnableTex)
{
	m_model_view = m_view;
	m_original_model_view = m_view;
	m_vars = vars;
	m_uEnableTex = uEnableTex;
}
...
void Drawer::drawWallCube(void)
{
	glBindTexture( GL_TEXTURE_2D, texture_floor );
	glUniform1i( m_uEnableTex, 1 );
    glUniformMatrix4fv( m_vars.uModelView, 1, GL_TRUE, m_model_view );
    glBindVertexArray( m_texCubeData.vao );
    glDrawArrays( GL_TRIANGLES, 0, m_texCubeData.numVertices );
	glUniform1i( m_uEnableTex, 0 );
}

void Drawer::drawFloorCube(void)
{
	glBindTexture( GL_TEXTURE_2D, texture_floor );
	glUniform1i( m_uEnableTex, 1 );
    glUniformMatrix4fv( m_vars.uModelView, 1, GL_TRUE, m_model_view );
...
void Drawer::drawTablePlane(void)
{
    ...
    m_model_view = m_original_model_view;
    m_model_view *= Translate(0.0f, 0.0f, z_trans-0.5f);
    m_model_view *= Scale(5.0f, 0.3f, 1.0f);
    drawCube();
...
void Drawer::drawFloor(void)
{
	set_colour(1.0f, 1.0f, 1.0f);
	m_model_view = m_original_model_view;
	m_model_view *= Translate(0.0f, -TABLE_DEPTH * 14.0f-3.0f, 0.0f);
	m_model_view *= Scale(ROOM_LENGTH, 1.0f, ROOM_WIDTH);
	drawWallCube();
}

void Drawer::drawWalls(void)
{
	// Wall on the left
	set_colour(1.0f, 1.0f, 1.0f);
	m_model_view = m_original_model_view;
	m_model_view *= Translate(-ROOM_LENGTH/2.0, 23.0f, 0.0f);
	m_model_view *= Scale(1.0f, ROOM_HEIGHT, ROOM_WIDTH);
	drawWallCube();
	...
	set_colour(1.0f, 1.0f, 1.0f);
	m_model_view = m_original_model_view;
	m_model_view *= Translate(0.0f, 23.0f, -ROOM_WIDTH/2.0f);
	m_model_view *= Scale(ROOM_LENGTH, ROOM_HEIGHT, 1.0f);
	drawWallCube();

	// Wall in the back
	set_colour(1.0f, 1.0f, 1.0f);
	m_model_view = m_original_model_view;
	m_model_view *= Translate(0.0, 23.0f, ROOM_WIDTH/2.0f);
	m_model_view *= Scale(ROOM_LENGTH, ROOM_HEIGHT, 1.0f);
	drawWallCube();
}

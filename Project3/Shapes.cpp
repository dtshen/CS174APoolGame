...
/********************************************************/
//the points to create the wall. its in the xy plane
point4 m_wall[6] = {point4(-1,1,0,1), point4(-1,-1,0,1), point4(1,-1,0,1), point4(-1,1,0,1), point4(1,-1,0,1), point4(1,1,0,1)};
//the normal to the wall
point3 wallNormal[6] = {point3(0.0f,0.0f,1.0f),point3(0.0f,0.0f,1.0f),point3(0.0f,0.0f,1.0f),point3(0.0f,0.0f,1.0f),point3(0.0f,0.0f,1.0f),point3(0.0f,0.0f,1.0f)};
//the uvs for the texture. they are the points used in all the faces of a cube
point2 wallUV[6] = {point2(0.0,1.0),point2(0.0f, 0.0f),point2(1.0f, 0.0f), point2(0.0f, 1.0f), point2(1.0f, 0.0f), point2(1.0f, 1.0f)};
void genWall(GLuint program, ShapeData* walls) {
	...
	setVertexAttrib(program, 
        (float*)m_wall,  sizeof(m_wall),
        (float*)wallNormal, sizeof(wallNormal),(float*)wallUV,sizeof(wallUV));
...
//    to the vertices
int Index = 0;
void quad( int a, int b, int c, int d, const point3& normal, float zoom )
{
    cubePoints[Index] = vertices[a]; cubeNormals[Index] = normal; 
    cubeUV[Index] = point2(0.5f-0.5f/zoom, 0.5f+0.5f/zoom); Index++;
...
void colorcube(float zoom)
{
    quad( 1, 0, 3, 2, point3( 0.0f,  0.0f,  1.0f), zoom );
    quad( 2, 3, 7, 6, point3( 1.0f,  0.0f,  0.0f), zoom );
    quad( 0, 4, 7, 3, point3( 0.0f, -1.0f,  0.0f), zoom );
    quad( 5, 1, 2, 6, point3( 0.0f,  1.0f,  0.0f), zoom );
    quad( 4, 5, 6, 7, point3( 0.0f,  0.0f, -1.0f), zoom );
    quad( 5, 4, 0, 1, point3(-1.0f,  0.0f,  0.0f), zoom );
}
...
void generateEdgeCorner(GLuint program, ShapeData* edgeCornerData)
{
	...
	for(i = 0; i <= numEdgeCornerDivisions; i++)
	{
		...
		innerPoints[i] = point2(sqrt(1)*cosf(a)-0.5f, -1.0f*sqrt(1)*sinf(a)-0.5f);
	}
